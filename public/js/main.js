...
const gameTime = 120000; // en millisecondes
let playerName = null;
let playerTime = 0;
let interval = null;

// Lancement d'une nouvelle partie lors du clic sur le bouton "Nouvelle partie"
newGameButton.addEventListener('click', item => {
    ...
    // On déclenche le compte à rebour
...
function cardClicked(event) {
    ...
    // c'est à dire celles avec la classe "card--flipped" mais sans la classe "card--flipped-right" ou "card--flipped-wrong"
    ...
    if (flippedCards.length === 2) {
        const firstCardNumber = flippedCards[0].getAttribute('data-card');
        const secondCardNumber = flippedCards[1].getAttribute('data-card');
